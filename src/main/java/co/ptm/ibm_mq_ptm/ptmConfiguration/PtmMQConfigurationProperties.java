...
@Configuration
@ConfigurationProperties(prefix = "ptm.ibm.mq")
public class PtmMQConfigurationProperties {
    private static Logger logger = LoggerFactory.getLogger(PtmMQConfigurationProperties.class);
    ...
    public PtmMQConfigurationProperties() {
    ...
    public void traceProperties(MQConnectionDetails connectionDetails) {
        ...
        if (!StringUtils.isEmpty(getSslBundle())) {
            ...
            logger.trace("JKS tru!StringUtils.isEmpty(ststore pw set  : {}", (pw != null && pw.length() > 0) ? "YES" : "NO");
        }
        else {
